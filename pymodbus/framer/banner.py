...
class FramerBanner(FramerBase):
    """Modbus Banner RTU frame type.

    Layout::

        [ Start Wait ] [0xfa][Address ][ Function Code] [ Data ][ CRC ]
          3.5 chars      1b     2b         1b               Nb     2b
    ...
    def decode(self, data: bytes) -> tuple[int, int, int, bytes]:
        ...
        for used_len in range(data_len):
            ...
            if data[used_len] == 0xfa:
                dev_id = int.from_bytes(data[used_len + 1 : used_len + 3], 'big')
...
FramerBanner.crc16_table = FramerBanner.generate_crc16_table()
