...
async def run_a_few_calls(client: modbusClient.AsyncModbusSerialClient):
    """Test connection works."""
    # rr = await client.read_coils(32, count=1, slave=1)
    # assert len(rr.bits) == 8
    rr = await client.read_holding_registers(6400, count=10, slave=56831)
    # assert rr.registers[0] == 17
    # assert rr.registers[1] == 17
    _logger.info(f"### rr={rr}")
    _logger.info(f"### rr.registers={rr.registers}")
    rr = await client.read_holding_registers(6400, count=10, slave=56159)
    # assert rr.registers[0] == 17
    # assert rr.registers[1] == 17
    _logger.info(f"### rr={rr}")
    _logger.info(f"### rr.registers={rr.registers}")

    rr = await client.write_register(6803, 51, slave=56831)
    _logger.info(f"### rr={rr}")
    rr = await client.write_register(6803, 52, slave=56159)
    _logger.info(f"### rr={rr}")
    rr = await client.write_registers(6501, [53, 100], slave=56831)
    _logger.info(f"### rr={rr}")
    rr = await client.read_holding_registers(6500, count=3, slave=51)
    _logger.info(f"### rr={rr}")
    offset = 100
    unit_id = 52
    reg_unit_id = 6502 + 100
    rr = await client.write_register(reg_unit_id, 56159, slave=56831)
    _logger.info(f"### rr={rr}")
    rr = await client.write_registers(100, [1,1], slave=56159)
    _logger.info(f"### rr={rr}")
    
    try:
        rr = await client.read_holding_registers(1, count=10, slave=offset + unit_id)
        _logger.info(f"### rr={rr}")
        _logger.info(f"### rr.registers={rr.registers}")
    except Exception as e:
        _logger.info(f"### Exception: {e}")

    rr = await client.write_registers(100, [0,0], slave=56159)
    _logger.info(f"### rr={rr}")    
